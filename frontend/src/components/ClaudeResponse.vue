// frontend/src/components/ClaudeResponse.vue
<template>
  <div>
  ...
  methods: {
    async getResponse() {
      ...
      try {
        ...
        console.log(data);
        // 处理返回的数据，例如更新聊天历史
        this.$emit('update-chat-history', [{ role: "user", content: this.value }, { role: "system", content: data.reply }]); //只用给历史记录中增加本轮的用户和AI的对话即可
